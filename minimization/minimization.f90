...
contains
  ...
  function numgrad(f,x) result(grad)
    interface
       function f(x)
         real*8 x(:), f
       end function f
    end interface
    real*8 x(:), grad(size(x)), fx
  ...
  subroutine quasi_newton(f,x,eps,steps)
    real*8 x(:), eps, lambda
    real*8, parameter :: dx=1.5d-8, alpha=1d-2
    interface
       function f(x)
         real*8 x(:), f
       end function f
    ...
    gx = numgrad(f,x) !So we don't have to call the functions multiple times inside the loop
    fx = f(x)      !Same as above
